...
int WebOTA_async::init(const unsigned int port, const char *path) {
	this->port = port;
	this->path = path;
...
// One param
int WebOTA_async::init(const unsigned int port) {
	return WebOTA_async::init(port, "/webota");
}

// No params
int WebOTA_async::init() {
	return WebOTA_async::init(8080, "/webota");
}

int WebOTA_async::handle() {
...
long WebOTA_async::max_sketch_size() {
...
#ifdef ESP8266
int WebOTA_async::add_http_routes(ESP8266WebServer *server, const char *path) {
#endif
#ifdef ESP32
int WebOTA_async::add_http_routes(WebServer *server, const char *path) {
#endif
	// Index page
...
void WebOTA_async::delay(unsigned int ms) {
...
void WebOTA_async::set_custom_html(char const * const html) {
